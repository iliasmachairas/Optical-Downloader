#!/usr/bin/env bash
#
# build_plugin.sh — package the Optical Downloader QGIS plugin into a clean
...
PLUGIN_DIR_NAME="$(basename "$SCRIPT_DIR")"          # e.g. optical_downloader
VERSION="$(grep -E '^version=' metadata.txt | head -1 | cut -d= -f2 | tr -d '[:space:]')"
ZIP_NAME="Optical-Downloader-${VERSION}.zip"
...
if [ "$DO_GIT" -eq 1 ]; then
  ...
  if git remote get-url origin >/dev/null 2>&1; then
    ...
    echo
    echo ">> No 'origin' remote set. Repo: https://github.com/iliasmachairas/Optical-Downloader"
    echo "   Run:"
    echo "     git remote add origin https://github.com/iliasmachairas/Optical-Downloader.git"
